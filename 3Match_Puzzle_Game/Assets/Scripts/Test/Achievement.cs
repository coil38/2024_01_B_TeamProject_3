...
public class Achievement                // 업적 클래스 선언 (모노 비해이비어(MonoBehaviour) X)
{
    public string name;                 // 업적 이름
    public string descripttion;         // 업적 설명
    public bool isUnlocked;             // 잠김 설정
    public int currentProgress;         // 진행 상태
    public int goal;                    // 완료 상태

    public Achievement(string name, string descripttion, int goal)      // 생성자 함수
    {
        this.name = name;                               // Achievement 클래스가 생성 될 때 이름을 인수로 받아서 설정
        this.descripttion = descripttion;               // Achievement 클래스가 생성 될 때 설명을 인수로 받아서 설정
        this.isUnlocked = false;                        // Achievement 클래스가 생성 될 때 false
        this.currentProgress = 0;                       // Achievement 클래스가 생성 될 때 0
        this.goal = goal;                               // Achievement 클래스가 생성 될 때 완료 숫자
    }
